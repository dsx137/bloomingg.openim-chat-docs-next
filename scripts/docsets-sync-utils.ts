...
function renderPrompt(records: DiffRecord[]): string {
  const lines: string[] = [
    ...
    '- Spawn exactly one subagent for every docset listed below and run those docset subagents in parallel. Give each subagent exclusive write ownership of its local docs root.',
