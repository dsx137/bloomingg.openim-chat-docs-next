...
使用 **获取用户 Token** 从可信后端调用 OpenIM 认证 REST 接口。由可信后端为指定用户和登录终端签发用户 Token，客户端使用该 Token 登录 SDK。 请先在[接入准备](/platform-api/prepare-to-use-api)中配置 API 地址；管理员 Token 只能保存在可信后端服务中，不能下发到客户端或写入前端代码。
