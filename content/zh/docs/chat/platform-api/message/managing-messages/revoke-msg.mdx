---
title: "撤回消息"
description: "OpenIM 消息 REST API：撤回消息。"
...
使用 **撤回消息** 从业务服务端调用 OpenIM 消息 REST 接口，按会话 ID 和 Seq 撤回一条已发送消息。撤回成功后，相关客户端会收到撤回同步事件。
...
- 撤回指定消息，并向相关用户同步撤回通知。
- `conversationID`、`seq` 和 `userID` 必填。
- 操作者必须具备撤回该消息的权限。
- 建议在业务侧记录撤回操作者、原因和原消息定位信息，便于审计。
