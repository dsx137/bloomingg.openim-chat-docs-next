---
title: "获取单个会话"
description: "OpenIM 会话 REST API：获取单个会话。"
sourcePath: "/docs/chat/platform-api/v3/conversation/listing-conversations/get-conversation"
---

使用 **获取单个会话** 从可信后端调用 OpenIM 会话 REST 接口。按会话 ID 和所属用户查询单个会话记录。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
...
| data | object | 接口返回数据。 |
| data.conversation | object | 会话详情，结构为[会话模块的 `ConversationInfo`](/docs/chat/platform-api/v3/conversation/overview#conversationinfo)。 |
...
- 只读查询，不改变会话状态。
- `conversationID` 和 `ownerUserID` 必填。
