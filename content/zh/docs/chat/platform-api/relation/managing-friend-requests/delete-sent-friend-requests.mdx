---
title: "删除发出的好友申请"
description: "OpenIM 商业版 Platform API：删除指定用户向其他用户发出的好友申请记录。"
...
version: "v3"
edition: "enterprise"
sourcePath: "/docs/chat/platform-api/v3/relation/managing-friend-requests/delete-sent-friend-requests"
---

删除指定用户向其他用户发出的好友申请记录。 此接口仅在 OpenIM 商业版中提供。请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和有效 Token。
...
| 参数名     | 是否必填 | 类型     | 说明                         |
| ---------- | -------- | -------- | ---------------------------- |
| fromUserID | 是       | string   | 好友申请发起者用户 ID。      |
| toUserIDs  | 是       | string[] | 好友申请接收者用户 ID 列表。 |
