...
使用 **申请加入群组** 从可信后端调用 OpenIM 群组 REST 接口。提交入群申请；如果群组允许直接加入，服务端会直接写入成员关系。 请先在[接入准备](/platform-api/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
