---
title: "禁言群组"
description: "OpenIM 群组 REST API：禁言群组。"
sourcePath: "/platform-api/group/group-moderation/mute-group"
---

使用 **禁言群组** 从可信后端调用 OpenIM 群组 REST 接口。禁言整个群组；群主和管理员通常仍可发言。 请先在[接入准备](/platform-api/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
...
| muteBypassUserIDs <span className="enterprise-field-badge">商业版</span> | 否 | string[] | 全群禁言期间仍允许发送消息的用户 ID 列表。 |
