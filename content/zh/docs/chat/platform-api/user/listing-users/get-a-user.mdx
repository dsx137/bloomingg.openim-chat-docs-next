...
使用 **获取用户** 从可信后端调用 OpenIM REST 接口。请先在[接入准备](/platform-api/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
...
| usersInfo | array  | [用户模块的 `UserInfo`](/platform-api/user/overview#userinfo) 列表 |
