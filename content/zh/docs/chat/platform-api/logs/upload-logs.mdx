...
使用 **上传日志记录** 从可信后端调用 OpenIM 日志接口。业务方可以先在客户端采集本地日志并上传到对象存储或自己的文件服务，再通过此接口登记日志文件 URL、平台、版本和扩展信息，便于后续在后台统一检索。请先在[接入准备](/platform-api/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
- [认证接口](/platform-api/auth/tokens/get-admin-token)
