---
title: "查询用户列表"
description: "OpenIM Platform API 中文参考：查询用户列表。说明 OpenIM REST 支持情况、接口结构和后端实现边界。"
sourcePath: "/docs/chat/platform-api/v3/user/listing-users/list-users"
---
...
const response = await fetch(`${API_ADDRESS}/user/get_users`, {
  method: 'POST',
  headers: {
...
func main() {
  ...
  "pagination": {
	...
	url := os.Getenv("OPENIM_API_ADDRESS") + "/user/get_users"
...
| statuses <span className="enterprise-field-badge">商业版</span> | 选填 | int[] | 按用户账号状态筛选，元素参见[用户模块的 `UserStatus`](/docs/chat/platform-api/v3/user/overview#userstatus)。 |
...
| total   | int    | 用户总数                                               |
| users   | array  | [用户模块的 `UserInfo`](/docs/chat/platform-api/v3/user/overview#userinfo) 列表 |
