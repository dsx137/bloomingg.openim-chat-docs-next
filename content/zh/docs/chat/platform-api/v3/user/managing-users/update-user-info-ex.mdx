---
title: "更新用户信息扩展"
description: "OpenIM 用户 REST API：更新用户信息扩展。"
sourcePath: "/docs/chat/platform-api/v3/user/managing-users/update-user-info-ex"
---

使用 **更新用户信息扩展** 从可信后端调用 OpenIM 用户 REST 接口。使用包装字段更新用户资料；只更新请求中显式传入的字段。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
