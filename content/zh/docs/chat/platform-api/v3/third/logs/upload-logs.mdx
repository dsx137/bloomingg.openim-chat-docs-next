---
title: "上传日志记录"
description: "OpenIM 第三方服务 REST API：上传日志记录。"
sourcePath: "/docs/chat/platform-api/v3/third/logs/upload-logs"
---

使用 **上传日志记录** 从可信后端调用 OpenIM 第三方服务 REST 接口。登记客户端或业务侧已上传的日志文件 URL，便于后续在后台检索。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
