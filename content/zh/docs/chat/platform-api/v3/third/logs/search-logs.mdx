---
title: "搜索日志记录"
description: "OpenIM 第三方服务 REST API：搜索日志记录。"
sourcePath: "/docs/chat/platform-api/v3/third/logs/search-logs"
---

使用 **搜索日志记录** 从可信后端调用 OpenIM 第三方服务 REST 接口。按关键字、时间范围和分页参数检索客户端日志记录。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
