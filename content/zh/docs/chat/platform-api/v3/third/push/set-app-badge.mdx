---
title: "设置应用角标"
description: "OpenIM 第三方服务 REST API：设置应用角标。"
sourcePath: "/docs/chat/platform-api/v3/third/push/set-app-badge"
---

使用 **设置应用角标** 从可信后端调用 OpenIM 第三方服务 REST 接口。设置指定用户在应用推送通道中的未读角标数量。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
- 更新用户在第三方推送服务中的应用角标数量。
- `userID` 和 `appUnreadCount` 必填。
- `appUnreadCount` 应大于等于 0。
