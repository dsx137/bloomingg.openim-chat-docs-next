---
title: "完成分片上传"
description: "OpenIM 第三方服务 REST API：完成分片上传。"
sourcePath: "/docs/chat/platform-api/v3/third/object-storage/complete-multipart-upload"
---

使用 **完成分片上传** 从可信后端调用 OpenIM 第三方服务 REST 接口。在所有分片上传到对象存储后，确认并生成 OpenIM 可访问的对象 URL。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
| data | object | 接口返回数据。 |
| data.url | string | 完成上传后 OpenIM 返回的对象访问 URL。 |
...
- 完成对象存储分片上传，并写入 OpenIM 对象记录。
- `uploadID`、`parts` 和 `name` 必填。
- 必须先完成所有分片上传，再调用此接口。
