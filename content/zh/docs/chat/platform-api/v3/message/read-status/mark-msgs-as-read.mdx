---
title: "标记多条消息已读"
description: "OpenIM 消息 REST API：标记多条消息已读。"
sourcePath: "/docs/chat/platform-api/v3/message/read-status/mark-msgs-as-read"
---

使用 **标记多条消息已读** 从可信后端调用 OpenIM 消息 REST 接口。将指定会话中的一组消息 Seq 标记为已读。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
- 更新消息已读状态，并可能触发已读同步通知。
- `conversationID`、`seqs` 和 `userID` 必填。
- `seqs` 不要传重复值。
