---
title: "设置会话已读序列号"
description: "OpenIM 消息 REST API：设置会话已读序列号。"
sourcePath: "/docs/chat/platform-api/v3/message/read-status/set-conversation-has-read-seq"
---

使用 **设置会话已读序列号** 从可信后端调用 OpenIM 消息 REST 接口。直接设置指定用户在某个会话中的已读 Seq。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
- 更新会话已读 Seq，并按参数决定是否下发通知。
- `conversationID`、`userID` 和 `hasReadSeq` 必填。
