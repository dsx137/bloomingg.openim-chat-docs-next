---
title: "标记会话已读"
description: "OpenIM 消息 REST API：标记会话已读。"
sourcePath: "/docs/chat/platform-api/v3/message/read-status/mark-conversation-as-read"
---

使用 **标记会话已读** 从可信后端调用 OpenIM 消息 REST 接口。设置指定用户在某个会话中的已读 Seq，可同时携带已读消息 Seq 列表。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
