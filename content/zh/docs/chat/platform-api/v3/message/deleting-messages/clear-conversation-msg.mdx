---
title: "清空会话消息"
description: "OpenIM 消息 REST API：清空会话消息。"
sourcePath: "/docs/chat/platform-api/v3/message/deleting-messages/clear-conversation-msg"
---

使用 **清空会话消息** 从可信后端调用 OpenIM 消息 REST 接口。清空指定用户在一个或多个会话中的消息，并重置最小 Seq。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
- 清空用户在指定会话中的服务端消息可见范围，并按同步选项下发删除同步。
- `conversationIDs` 和 `userID` 必填。
