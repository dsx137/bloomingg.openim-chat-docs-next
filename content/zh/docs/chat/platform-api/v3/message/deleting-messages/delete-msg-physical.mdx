---
title: "按时间物理删除消息"
description: "OpenIM 消息 REST API：按时间物理删除消息。"
sourcePath: "/docs/chat/platform-api/v3/message/deleting-messages/delete-msg-physical"
---

使用 **按时间物理删除消息** 从可信后端调用 OpenIM 消息 REST 接口。按会话 ID 列表和时间戳从服务端物理删除早于指定时间的消息。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
