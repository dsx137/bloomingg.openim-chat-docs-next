---
title: "清空用户全部消息"
description: "OpenIM 消息 REST API：清空用户全部消息。"
sourcePath: "/docs/chat/platform-api/v3/message/deleting-messages/user-clear-all-msg"
---

使用 **清空用户全部消息** 从可信后端调用 OpenIM 消息 REST 接口。清空指定用户全部会话消息，并重置相关最小 Seq。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
- 清空用户全部会话的服务端消息可见范围，并按同步选项下发删除同步。
- `userID` 必填。
- 该操作影响范围大，调用前应在业务侧二次确认。
