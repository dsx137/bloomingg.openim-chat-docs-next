...
使用 **获取服务器时间** 从可信后端调用 OpenIM 消息 REST 接口。获取 OpenIM 消息服务当前服务器时间。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
