---
title: "批量发送消息"
description: "OpenIM 消息 REST API：批量发送消息。"
sourcePath: "/docs/chat/platform-api/v3/message/sending-messages/batch-send-msg"
---

使用 **批量发送消息** 从可信后端调用 OpenIM 消息 REST 接口。由 APP 管理员向多个用户批量发送同一条消息，或向全量用户发送。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
