...
使用 **发送业务通知** 从可信后端调用 OpenIM 消息 REST 接口。发送一条自定义业务通知，客户端通过业务通知回调接收 `key` 和 `data`。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
