---
title: "强制用户下线"
description: "OpenIM 认证 REST API：强制用户下线。"
sourcePath: "/docs/chat/platform-api/v3/auth/sessions/force-logout"
---

使用 **强制用户下线** 从可信后端调用 OpenIM 认证 REST 接口。强制指定用户从某个终端退出登录，客户端 SDK 会收到被踢下线回调。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址；管理员 Token 只能保存在可信后端服务中，不能下发到客户端或写入前端代码。
...
| 参数名 | 是否必填 | 类型 | 说明 |
| --- | --- | --- | --- |
| platformID | 是 | int | 需要踢下线的终端类型，参见[用户模块的 `PlatformID`](/docs/chat/platform-api/v3/user/overview#platformid)。 |
...
- 将指定用户在指定终端的 Token 标记为 kicked，并通知网关断开对应连接。
- 必须使用 APP 管理员 Token 调用。
- `platformID` 和 `userID` 必填。
