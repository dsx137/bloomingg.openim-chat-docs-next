---
title: "获取会话离线推送用户 ID"
description: "OpenIM 会话 REST API：获取会话离线推送用户 ID。"
sourcePath: "/docs/chat/platform-api/v3/conversation/conversation-state/get-conversation-offline-push-user-ids"
---

使用 **获取会话离线推送用户 ID** 从可信后端调用 OpenIM 会话 REST 接口。在指定候选用户中，查询某个会话仍应接收离线推送的用户 ID。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
...
| data | object | 接口返回数据。 |
| data.userIDs | array | 需要接收离线推送的用户 ID 列表。 |
