---
title: "查询指定好友申请"
description: "OpenIM 好友 REST API：查询指定好友申请。"
sourcePath: "/docs/chat/platform-api/v3/friend/managing-friend-requests/get-designated-friend-application"
---

使用 **查询指定好友申请** 从可信后端调用 OpenIM 好友 REST 接口。查询两个用户之间指定方向的好友申请记录。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。

## HTTP 请求

```bash
POST {API_ADDRESS}/friend/get_designated_friend_apply
```

### 请求示例

```bash
curl --request POST "${API_ADDRESS}/friend/get_designated_friend_apply" \
...
| data | object | 接口返回数据。 |
| data.friendRequests | array | 好友申请列表。 |
