---
title: "设置好友备注"
description: "OpenIM 好友 REST API：设置好友备注。"
sourcePath: "/docs/chat/platform-api/v3/friend/managing-friends/set-friend-remark"
---

使用 **设置好友备注** 从可信后端调用 OpenIM 好友 REST 接口。设置或清空某个好友的备注。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。

## HTTP 请求

```bash
POST {API_ADDRESS}/friend/set_friend_remark
```

### 请求示例

```bash
curl --request POST "${API_ADDRESS}/friend/set_friend_remark" \
...
- 更新好友备注，并向相关用户下发好友资料变更通知。
- `ownerUserID` 和 `friendUserID` 必填。
