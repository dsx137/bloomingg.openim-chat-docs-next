...
Join group is an OpenIM group REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| reqMessage | No | string | The req message value. |
| joinSource | Yes | int | The join source value. |
| inviterUserID | Yes | string | The inviter user id value. |
