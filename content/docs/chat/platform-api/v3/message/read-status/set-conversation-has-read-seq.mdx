---
title: "Set conversation read sequence"
description: "Set conversation read sequence with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/message/read-status/set-conversation-has-read-seq"
---

Set conversation read sequence is an OpenIM message REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/msg/set_conversation_has_read_seq
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/msg/set_conversation_has_read_seq" \
...
| hasReadSeq | Yes | int | Read sequence number. |
| noNotification | No | boolean | The no notification value. |
