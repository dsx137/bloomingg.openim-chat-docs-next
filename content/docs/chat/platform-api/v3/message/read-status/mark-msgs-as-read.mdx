...
description: "Mark messages as read with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/message/read-status/mark-msgs-as-read"
---

Mark messages as read is an OpenIM message REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/msg/mark_msgs_as_read
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/msg/mark_msgs_as_read" \
