---
title: "Get latest sequence"
description: "Get latest sequence with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/message/retrieving-messages/newest-seq"
---

Get latest sequence is an OpenIM message REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/msg/newest_seq
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/msg/newest_seq" \
...
```json
{
  ...
  "data": {
    "maxSeqs": {
      "si_user_001_user_002": 128
    },
    "minSeqs": {
      "si_user_001_user_002": 1
    }
  }
}
```
...
| data.maxSeqs | object | The max seqs value. |
| data.minSeqs | object | The min seqs value. |
