---
title: "Check message is send success"
description: "Check message is send success with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/message/sending-messages/check-msg-is-send-success"
---

Check message is send success is an OpenIM message REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/msg/check_msg_is_send_success
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/msg/check_msg_is_send_success" \
...
| data.status | int | Status value returned by OpenIM. |
