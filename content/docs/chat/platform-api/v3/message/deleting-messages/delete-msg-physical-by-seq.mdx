---
title: "Physically delete messages by sequence"
description: "Physically delete messages by sequence with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/message/deleting-messages/delete-msg-physical-by-seq"
---

Physically delete messages by sequence is an OpenIM message REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/msg/delete_msg_phsical_by_seq
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/msg/delete_msg_phsical_by_seq" \
