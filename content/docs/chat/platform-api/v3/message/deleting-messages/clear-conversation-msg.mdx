---
title: "Clear conversation message"
description: "Clear conversation message with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/message/deleting-messages/clear-conversation-msg"
---

Clear conversation message is an OpenIM message REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/msg/clear_conversation_msg
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/msg/clear_conversation_msg" \
...
| deleteSyncOpt | No | object | The delete sync opt value. |
| deleteSyncOpt.IsSyncSelf | No | boolean | The is sync self value. |
| deleteSyncOpt.IsSyncOther | No | boolean | The is sync other value. |
