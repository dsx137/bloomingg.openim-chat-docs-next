---
title: "User clear all message"
description: "User clear all message with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/message/deleting-messages/user-clear-all-msg"
---

User clear all message is an OpenIM message REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/msg/user_clear_all_msg
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/msg/user_clear_all_msg" \
