...
Update friends is an OpenIM relationship REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| friendUserIDs | Yes | array | The friend user ids value. |
| isPinned | No | object | The is pinned value. |
| remark | No | object | Friend remark. |
| ex | No | object | Business extension field. |
