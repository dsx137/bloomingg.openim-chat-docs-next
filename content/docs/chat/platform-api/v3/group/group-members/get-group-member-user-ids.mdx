...
description: "Get group member user IDs with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/group/group-members/get-group-member-user-ids"
---

Get group member user IDs is an OpenIM group REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/group/get_group_member_user_id
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/group/get_group_member_user_id" \
