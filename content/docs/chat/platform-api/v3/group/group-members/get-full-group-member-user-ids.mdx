---
title: "Get full group member user IDs"
description: "Get full group member user IDs with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/group/group-members/get-full-group-member-user-ids"
---

Get full group member user IDs is an OpenIM group REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/group/get_full_group_member_user_ids
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/group/get_full_group_member_user_ids" \
