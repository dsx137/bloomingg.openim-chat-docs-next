...
使用 **获取群成员列表** 从可信后端调用 OpenIM 群组 REST 接口。按分页、角色过滤和关键字查询群成员列表。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
