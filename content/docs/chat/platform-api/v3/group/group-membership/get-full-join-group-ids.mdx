...
description: "Get full join group IDs with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/group/group-membership/get-full-join-group-ids"
---

Get full join group IDs is an OpenIM group REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| data.groupIDs | array | List of OpenIM group IDs. |
