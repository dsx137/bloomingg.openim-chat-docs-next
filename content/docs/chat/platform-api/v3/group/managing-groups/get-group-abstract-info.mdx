...
description: "Get group abstract info with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/group/managing-groups/get-group-abstract-info"
---

Get group abstract info is an OpenIM group REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/group/get_group_abstract_info
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/group/get_group_abstract_info" \
...
| data.groupAbstractInfos | array | The group abstract infos value. |
