...
description: "Get all conversations with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/conversation/listing-conversations/get-all-conversations"
---

Get all conversations is an OpenIM conversation REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/conversation/get_all_conversations
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/conversation/get_all_conversations" \
...
| data.conversations | array | The conversations value. |
| data.conversations[].ownerUserID | string | Group owner user ID. |
| data.conversations[].conversationID | string | OpenIM conversation ID. |
| data.conversations[].conversationType | int | The conversation type value. |
| data.conversations[].userID | string | OpenIM user ID. |
| data.conversations[].groupID | string | OpenIM group ID. |
| data.conversations[].isPinned | boolean | The is pinned value. |
| data.conversations[].recvMsgOpt | int | The recv msg opt value. |
| data.conversations[].ex | string | Business extension field. |
