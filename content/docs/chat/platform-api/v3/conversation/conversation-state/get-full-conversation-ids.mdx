---
title: "Get full conversation IDs"
description: "Get full conversation IDs with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/conversation/conversation-state/get-full-conversation-ids"
---

Get full conversation IDs is an OpenIM conversation REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/conversation/get_full_conversation_ids
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/conversation/get_full_conversation_ids" \
...
| idHash | No | int | The id hash value. |
...
| data.version | int | The version value. |
| data.versionID | string | The version id value. |
| data.equal | boolean | The equal value. |
| data.conversationIDs | array | List of OpenIM conversation IDs. |
