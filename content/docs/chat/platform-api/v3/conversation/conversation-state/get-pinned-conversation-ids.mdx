...
description: "Get pinned conversation IDs with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/conversation/conversation-state/get-pinned-conversation-ids"
---

Get pinned conversation IDs is an OpenIM conversation REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/conversation/get_pinned_conversation_ids
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/conversation/get_pinned_conversation_ids" \
