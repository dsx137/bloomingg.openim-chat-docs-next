---
title: "注销全部会话 Token"
description: "OpenIM Platform API 中文参考：注销全部会话 Token。说明 OpenIM REST 支持情况、接口结构和后端实现边界。"
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/user/managing-session-tokens/revoke-all-session-tokens"
---

使用 **注销全部会话 Token** 从可信后端调用 OpenIM REST 接口。请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
...
const response = await fetch(`${API_ADDRESS}/auth/force_logout`, {
  method: 'POST',
  headers: {
...
func main() {
  ...
  "userID": "4950983283"
	...
	url := os.Getenv("OPENIM_API_ADDRESS") + "/auth/force_logout"
...
| token       | eyJhbxxxx3Xs  | 必填 | string | [管理员 token](./getAdminToken)                 |
...
| 参数名 | 是否必填 | 类型   | 说明                                                                                                              |
| :--------- | :--- | :----- | ----------------------------------------------------------------------------------------------------------------- |
| platformID | 必填 | int    | [用户登录时的终端类型](https://docs.openim.io/restapi/commonFields) |
