...
Search notification accounts is an OpenIM user REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| appManagerLevel | No | int | The app manager level value. |
...
| notificationAccounts | array | The notification accounts value. |
