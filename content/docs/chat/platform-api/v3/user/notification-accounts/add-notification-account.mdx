---
title: "新增系统通知账号"
description: "OpenIM 用户 REST API：新增系统通知账号。"
...
使用 **新增系统通知账号** 从可信后端调用 OpenIM 用户 REST 接口。创建系统通知账号或机器人账号；这类账号通常不用于客户端登录。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
