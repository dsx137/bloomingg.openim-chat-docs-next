---
title: "获取用户在线状态"
description: "OpenIM 用户 REST API：获取用户在线状态。"
...
使用 **获取用户在线状态** 从可信后端调用 OpenIM 用户 REST 接口。从消息网关聚合查询指定用户的在线状态和在线终端详情。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。
...
| 参数名 | 是否必填 | 类型 | 说明 |
| --- | --- | --- | --- |
| userIDs | 是 | array | 要查询在线状态的用户 ID 列表。 |
