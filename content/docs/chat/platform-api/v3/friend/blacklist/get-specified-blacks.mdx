---
title: "获取指定黑名单"
description: "OpenIM 好友 REST API：获取指定黑名单。"
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/friend/blacklist/get-specified-blacks"
---

使用 **获取指定黑名单** 从可信后端调用 OpenIM 好友 REST 接口。批量查询指定用户黑名单中的用户资料。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。

## HTTP 请求

```bash
POST {API_ADDRESS}/friend/get_specified_blacks
```

### 请求示例

```bash
curl --request POST "${API_ADDRESS}/friend/get_specified_blacks" \
