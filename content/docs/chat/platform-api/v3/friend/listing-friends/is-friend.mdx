---
title: "校验好友关系"
description: "OpenIM 好友 REST API：校验好友关系。"
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/friend/listing-friends/is-friend"
---

使用 **校验好友关系** 从可信后端调用 OpenIM 好友 REST 接口。检查两个用户是否互在对方好友列表中。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。

## HTTP 请求

```bash
POST {API_ADDRESS}/friend/is_friend
```

### 请求示例

```bash
curl --request POST "${API_ADDRESS}/friend/is_friend" \
