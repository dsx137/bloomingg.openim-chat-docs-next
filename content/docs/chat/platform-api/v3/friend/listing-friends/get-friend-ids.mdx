---
title: "获取好友 ID 列表"
description: "OpenIM 好友 REST API：获取好友 ID 列表。"
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/friend/listing-friends/get-friend-ids"
---

使用 **获取好友 ID 列表** 从可信后端调用 OpenIM 好友 REST 接口。获取指定用户的全部好友用户 ID。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和管理员 Token；接口参数通过请求头和 JSON 请求体传递。

## HTTP 请求

```bash
POST {API_ADDRESS}/friend/get_friend_id
```

### 请求示例

```bash
curl --request POST "${API_ADDRESS}/friend/get_friend_id" \
...
| data | object | 接口返回数据。 |
| data.friendIDs | array | 好友用户 ID 列表。 |
