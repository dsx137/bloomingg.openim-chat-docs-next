---
title: "Upload logs"
description: "Upload logs with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/third/logs/upload-logs"
---

Upload logs is an OpenIM third-party service REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| --- | --- | --- | --- |
| platform | Yes | int | The platform value. |
| fileURLs | Yes | array | The file urls value. |
| fileURLs[].filename | Yes | string | File name. |
| fileURLs[].URL | Yes | string | Resource URL. |
| appFramework | No | string | The app framework value. |
| version | No | string | The version value. |
