---
title: "删除日志记录"
description: "OpenIM 第三方服务 REST API：删除日志记录。"
...
使用 **删除日志记录** 从可信后端调用 OpenIM 第三方服务 REST 接口。按日志 ID 删除后台保存的日志索引记录。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
