---
title: "Set app badge"
description: "Set app badge with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/third/push/set-app-badge"
---

Set app badge is an OpenIM third-party service REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/third/set_app_badge
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/third/set_app_badge" \
...
| appUnreadCount | Yes | int | The app unread count value. |
