---
title: "更新 FCM Token"
description: "OpenIM 第三方服务 REST API：更新 FCM Token。"
...
使用 **更新 FCM Token** 从可信后端调用 OpenIM 第三方服务 REST 接口。更新指定账号在某个终端上的 FCM 推送 Token 及过期时间。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
...
- 更新第三方推送 Token 缓存或存储记录。
- `platformID`、`fcmToken`、`account` 和 `expireTime` 必填。
