...
description: "FCM update token with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
version: "v3"
sourcePath: "/docs/chat/platform-api/v3/third/push/fcm-update-token"
---

FCM update token is an OpenIM third-party service REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.

## HTTP request

```bash
POST {API_ADDRESS}/third/fcm_update_token
```

### Request example

```bash
curl --request POST "${API_ADDRESS}/third/fcm_update_token" \
...
| --- | --- | --- | --- |
| platformID | Yes | int | Client platform ID. |
| fcmToken | Yes | string | The fcm token value. |
| account | Yes | string | The account value. |
| expireTime | Yes | int | The expire time value. |
