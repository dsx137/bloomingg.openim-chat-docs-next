---
title: "获取分片上传限制"
description: "OpenIM 第三方服务 REST API：获取分片上传限制。"
...
使用 **获取分片上传限制** 从可信后端调用 OpenIM 第三方服务 REST 接口。获取对象存储分片上传允许的最小分片、最大分片和最大分片数量。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
