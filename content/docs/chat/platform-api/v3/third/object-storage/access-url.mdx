---
title: "获取对象访问地址"
description: "OpenIM 第三方服务 REST API：获取对象访问地址。"
...
使用 **获取对象访问地址** 从可信后端调用 OpenIM 第三方服务 REST 接口。为已保存的对象生成临时可访问 URL，可附带图片处理 query 参数。 请先在[接入准备](/docs/chat/platform-api/v3/prepare-to-use-api)中配置 API 地址和 Token；接口参数通过请求头和 JSON 请求体传递。
