---
title: "Refresh multipart upload signature"
description: "Refresh multipart upload signature with the OpenIM Platform API, including endpoint, request body, response fields, and error handling."
...
Refresh multipart upload signature is an OpenIM third-party service REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| partNumbers | Yes | array | The part numbers value. |
...
| data.query | array | The query value. |
| data.query[].key | string | The key value. |
| data.query[].values | array | The values value. |
| data.header | array | The header value. |
| data.header[].key | string | The key value. |
| data.header[].values | array | The values value. |
| data.parts | array | The parts value. |
| data.parts[].partNumber | int | The part number value. |
| data.parts[].url | string | Resource URL. |
| data.parts[].query | array | The query value. |
| data.parts[].query[].key | string | The key value. |
| data.parts[].query[].values | array | The values value. |
| data.parts[].header | array | The header value. |
| data.parts[].header[].key | string | The key value. |
| data.parts[].header[].values | array | The values value. |
