...
Add a notification account is an OpenIM user REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| nickName | Yes | string | User nickname. |
| faceURL | No | string | Avatar or icon URL. |
| appMangerLevel | Yes | int | Application manager level returned by OpenIM. |
