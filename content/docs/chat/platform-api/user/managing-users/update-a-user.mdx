...
Update a user is an OpenIM user REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| --- | --- | --- | --- |
| userInfo | Yes | object | The user info value. |
| userInfo.userID | Yes | string | OpenIM user ID. |
| userInfo.nickname | No | string | User nickname. |
| userInfo.faceURL | No | string | Avatar or icon URL. |
| userInfo.ex | No | string | Business extension field. |
| userInfo.globalRecvMsgOpt | No | int | Global message receiving option. |
