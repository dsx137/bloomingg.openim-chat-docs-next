...
Respond to a friend request is an OpenIM relationship REST API for trusted backend services. Configure the API address and administrator token in [Prepare to use the API](/docs/chat/platform-api/v3/prepare-to-use-api) before calling management endpoints.
...
| toUserID | Yes | string | The to user id value. |
| handleResult | Yes | int | The handle result value. |
| handleMsg | No | string | The handle msg value. |
